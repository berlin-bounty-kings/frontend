...
/* SBF Libraries */
import { SBFDataTypes } from "./libraries/SBFDataTypes.sol";
import { SBFErrors } from "./libraries/SBFErrors.sol";
import { SBFEvents } from "./libraries/SBFEvents.sol";
...
contract SBFModule is AccessControl {
	...
	function claimBounty(string memory _bountyId) external {
		...
		if (bountyInfoOf[_bountyId].bountyIs == SBFDataTypes.BountyIs.PAYED)
		...
		token.safeTransferFrom(
			address(this),
			msg.sender,
			bountyInfoOf[_bountyId].amount
		);

		// Emit event that the bounty has been claimed
		emit SBFEvents.BountyPayed(msg.sender, bountyInfoOf[_bountyId]);
